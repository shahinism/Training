...
You must write a module file to do most of the work. The module must export a
 ...
 argument must be the same as was passed to your program . i.e. don't turn it in
 to a RegExp or prefix with "." or do anything else but pass it to your module
...
The callback function must be called using the idiomatic node(err, data)
 ...
 data. In this case the data will be your filtered list of files, as an
 Array. If you receive an error, e.g. from your call to fs.readdir(), the
 callback must be called with the error, and only the error, as t he first
 argument.
...
The benifit of having a contract is that your module can be used by anyone who
expects this contract. So your module could be used by anyone els$ who does
...
reading and filtering function. To define a single function export $ou assign
...
-------------------------------------------------------------------------------*/
var readdirmod = require('./readdirmodule.js')
, directory = process.argv[2].toString()
, filter = process.argv[3].toString();

readdirmod(directory, filter, function(err, content){
    for(file in content)
